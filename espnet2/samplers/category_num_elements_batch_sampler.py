...
class CategoryNumElementsBatchSampler(AbsSampler):
    def __init__(
        self,
        batch_bins: int,
        ...
        self.utt2category_file = utt2category_file
        ...
        assert Path(utt2category_file).exists()
        category2utt = defaultdict(list)
        if utt2category_file is not None:
            utt2category = read_2column_text(utt2category_file)
            if len(set(first_utt2shape) - set(utt2category)) != 0:
                raise RuntimeError(
        ...
        keys = category2utt[self.category]
